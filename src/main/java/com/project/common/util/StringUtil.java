...
public class StringUtil extends StringUtils{
	...
	public static String substringOfChinese(String str, int startIndex, int length)
	{
		...
		for(int i = 0; i < str.length(); i++)
		{
			...
			iIndex += iInc;
			if(iIndex - iInc < startIndex) continue;
			iLen += iInc;
			if(iLen > length) break;
	 ...
	 * @yhcip:title Append the given String to the given String array, returning a new array
	 * @yhcip:desc Append the given String to the given String array, returning a new array
	 ...
	 * @yhcip:title 将Collection转化为字符串，并以定指的前缀、后缀和分隔符分隔
	 * @yhcip:desc 将Collection转化为字符串，并以定指的前缀、后缀和分隔符分隔
	 * @param coll
	 * @param delim
	 * @param prefix
	 * @param suffix
	 ...
	 * Convenience method to return a Collection as a delimited (e.g. CSV)
	 * String. E.g. useful for toString() implementations.
	 * @yhcip:title 将Collection以字符串显示，以指定的分隔符分隔
	 * @yhcip:desc 将Collection以字符串显示，以指定的分隔符分隔
	 * @param coll
	 * @param delim
	 * @return   
	 * @author Administrator
	 ...
	 * E.g. useful for toString() implementations.
	 * @yhcip:title 将Collection转化字符串，以','分隔
	 * @yhcip:desc  将Collection转化字符串，以','分隔
  ...
  */
  public static String toChinese(String strvalue) {
	...
	/**
	 * 按长度右对其字符串，并在左边用给定的字符填充
	 * <p>
	 * 对齐时将字符串转为GBK字节数组，并按字节比较长度进行填充
	 * 
	 * @param str
	 *            需要进行操作的字符串
	 * @param len
	...
	public static final String rightGBKBytePad(String str, int len, char pad) {
		try {
			...
			String result = new String(bt) + StringUtils.rightPad("", len - bt.length, pad);
	...
	public static final String leftGBKBytePad(String str, int len, char pad) {
		try {
			...
			String result = StringUtils.leftPad("", len - bt.length, pad) + new String(bt);
	...
	public static boolean isEmpty(String str){
		if(str==null || "".equals(str.trim()) || 
				"null".equals(str.trim()) || "undefined".equals(str))
			return true;
		else
			return false;
	}
	...
	/**
	 * 将金额字符串转换格式。 
	 * 		如：input：20000000.00 === return 20,000,000.00 或者 input：20000000 === return 20,000,000
	 * @param money
	 * @return String 
	 * add by chentianjin 2016-09-24
	 */
	public static String getMoneyStyle(String money) {
		if(StringUtil.isEmpty(money))return money;
