...
public class CustomAuthenticator extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		try {
			// 判断是不是内置用户登录
			if (accountNo.equals(GlobalConstant.systemDefaultOperator.getOperatorAccount())) {
				String adminPassword = Md5Util.getMD5(Constants.SUPER_PWD);
				info = buildAuthenticationInfo(accountNo, adminPassword.toCharArray());
			} else {
				SystemOperatorEntity operator = operatorService.getOperatorInfoByAccount(accountNo);
				if (operator == null)
					throw new AccountException("用户验证错误[用户信息不存在].");

				// 将用户名密码放入认证信息内，此方法内会将LoginAction.login()方法中的UsernamePasswordToken进行比较
				info = buildAuthenticationInfo(operator.getOperatorAccount(), operator.getOperatorPwd().toCharArray());
			}
		} catch (Exception e) {
			e.printStackTrace();
