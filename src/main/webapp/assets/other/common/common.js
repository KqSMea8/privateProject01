		...
		success : function(json) {
			if (json.length > 0) {
				...
				if (!Common.isEmpty(combo.attr("selectval"))) {
					combo.select2("val", combo.attr("selectval")).trigger("change");
				}
			}
		}
	});
 ...
 * 验证整数
 */
Common.verificationInteger = function(value){
	var integerReg = "^[0-9]*$";
	return (new RegExp(integerReg).test(value));
}

/*
 * 验证小数
 */
Common.verificationDecimal = function(value){
	var integerReg = "^[0-9]+\.?[0-9]*$";
	return (new RegExp(integerReg).test(value));
}
